...
New in v12.1: **index quality enforcement and phase gates** — fixes three critical bugs discovered in production: (1) tree-sitter symbol name truncation caused by agent-generated extraction scripts using manual byte slicing instead of `node.text` — replaced with a static, tested Python script (`scripts/extract_symbols.py`) that ships with the plugin and validates every symbol against its source line with zero tolerance (any corrupt symbol = fail the file and fix the script); (2) dead-end/dead-start resolution never ran — 41K dead ends were identified but 0 resolved, the `unresolved_connections` table was empty — hardened the completion gate so `ENRICHED_INDEX_COMPLETE` cannot be returned with any unresolved dead ends; (3) no human approval between phases — pipeline advanced from Phase 1 to Phase 4 without quality verification — added mandatory `AskUserQuestion` gates at every phase transition presenting quality stats for user approval. Also adds an index quality gate (Step 1c.5) that stops the pipeline if call resolution rate is below 50% (indicating corrupt index data), database schema integration in the graph builder (MySQL stored procs/triggers as entry points, table tracing in pathways, FK/trigger chain connections), and completion verification requirements for all status returns.
