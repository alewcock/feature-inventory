...
description: >
  Phase 3 orchestrator: annotates every pathway in the outcome graph with dimensional
  information (data, auth, logic, UI, config, side effects). Spawns pathway annotator
  agents in batches, grouped by entry point, then merges annotations into graph.db.
  Output: graph.db enriched with pathway_annotations and annotation_source_maps tables
  — ready for feature derivation (Phase 4).
