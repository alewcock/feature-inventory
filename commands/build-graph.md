...
description: >
  Phase 2 orchestrator: builds the outcome graph from the enriched index. Spawns
  graph builder agent(s) to identify entry points, final outcomes, and trace pathways,
  then validates the graph and interviews the user about orphans, unreachable outcomes,
  and graph gaps. Output: graph.db enriched with entry_points, final_outcomes, pathways,
  pathway_steps, fan_out_points, infrastructure, and graph_validation tables — ready
  for pathway annotation (Phase 3).
